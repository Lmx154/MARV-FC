...
| Protocol/comms split | `transitional` | `common/src/protocol/` and `common/src/comms/` exist, but preserved LoRa code remains under `common/src/coms/transport/lora/`. | Link transport code consolidated under `common/src/comms/links/`. | Perform the LoRa path migration as a separate follow-up change. |
