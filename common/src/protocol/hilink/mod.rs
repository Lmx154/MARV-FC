...
pub use rf::{
    ...
    lora_request_flags, lora_scaling, lora_state,
};
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct MixerMotorOrderPayload {
...
pub mod response_flags {
    ...
    pub const CONTROL_VALID: u32 = 1 << 4;
    pub const CONTROL_CLAMPED: u32 = 1 << 5;
    pub const SENSOR_INPUT_VALID: u32 = 1 << 6;
    pub const BENCH_OVERRIDE: u32 = 1 << 7;
}
...
pub mod mixer_motor_order {
    pub const IDENTITY: [u8; 4] = [1, 2, 3, 4];
}
...
impl WirePayload for MixerMotorOrderPayload {
    const MSG_TYPE: MsgType = MsgType::MixerMotorOrder;
    const WIRE_LEN: usize = 4;

    fn encode_payload(&self, out: &mut [u8]) -> Result<usize> {
        let mut w = Writer::new(out);
        w.bytes(&self.output_for_motor)?;
        Ok(w.len())
    }
...
impl WirePayload for ActuatorStatusPayload {
    ...
    fn encode_payload(&self, out: &mut [u8]) -> Result<usize> {
        ...
        w.bytes(&self.mixer_motor_order)?;
        w.u32(self.flags)?;
        Ok(w.len())
    }
...
mod tests {
    ...
    fn bench_payloads_round_trip() {
        ...
        round_trip_payload(
            &MixerMotorOrderPayload {
                output_for_motor: [3, 1, 4, 2],
            },
            26,
            2_006,
        );
        round_trip_payload(&ActuatorStatusRequestPayload, 27, 2_007);
        round_trip_payload(
    ...
    fn unsupported_header_version_is_rejected() {
        let ready = HilReadyPayload;
        let mut header = Header::new(MsgType::HilReady, 9, 2222, 0);
    ...
    #[test]
    fn rf_dialect_ids_are_not_normal_hilink_msg_types() {
