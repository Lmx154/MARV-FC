//! Generic ESKF engine over manifold nominal states.

use core::marker::PhantomData;

use super::base::{ErrorStateProcessModel, MeasurementModel, NominalState};
use super::gaussian::{
...
pub struct GenericEskf<S, C, P, T: EstimatorScalar, const N: usize, const W: usize>
where
    S: NominalState,
    P: ErrorStateProcessModel<S, C, T, N, W>,
{
    pub process_model: P,
    pub state: S,
...
impl<S, C, P, T: EstimatorScalar, const N: usize, const W: usize> GenericEskf<S, C, P, T, N, W>
where
    S: NominalState,
    P: ErrorStateProcessModel<S, C, T, N, W>,
{
    ...
    pub fn predict(
        ...
        let transition_jacobian =
            self.process_model
                .error_state_jacobian(&prior_state, control, dt);
        ...
        Model: MeasurementModel<S, T, N, M>,
    {
        ...
        let error_state_correction = kalman_gain * diagnostics.innovation;
        ...
        self.state = self
            .process_model
            .inject(&self.state, &error_state_correction)
            .copy();
        let reset_jacobian = self.process_model.reset_jacobian(&error_state_correction);
        self.covariance = regularize_covariance::<T, N>(
            &(reset_jacobian * updated_covariance * reset_jacobian.transpose()),
            &self.covariance_convention,
            true,
        );
        ...
        Ok(MeasurementUpdateSummary {
            ...
            innovation_covariance: Some(diagnostics.innovation_covariance),
            state_correction: Some(error_state_correction),
            gate_reason: None,
            diagnostics: Some(diagnostics),
        })
    }
