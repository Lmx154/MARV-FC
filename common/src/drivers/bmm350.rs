    ...
    I2C: I2c,
{
    ...
    pub async fn init<D: DelayMs>(&mut self, delay: &mut D) -> Result<(), Error> {
        ...
        while retries > 0 {
            match self.read_register(registers::CHIP_ID).await {
                Ok(id) => {
                    chip_id = id;
                    if self.verbose {
                        debug!("BMM350: CHIP_ID=0x{:02X}", id);
                    }
                    if id == 0x33 {
                        break;
                    }
                ...
                Err(_) => {
                    if self.verbose {
                        debug!("BMM350: CHIP_ID read failed, retry");
                    }
                }
            }
        ...
        // Enable XYZ
        self.write_register(registers::PMU_CMD_AXIS_EN, 0x07)
            .await?;
        delay.delay_ms(1).await;
        self.wait_not_busy(delay, 20).await?;

        // Set regular preset 25 Hz (example 0x16)
        self.write_register(registers::PMU_CMD_AGGR_SET, 0x16)
            .await?;
        delay.delay_ms(1).await;
        self.wait_not_busy(delay, 20).await?;
    ...
    pub async fn read_raw<D: DelayMs>(&mut self, delay: &mut D) -> Result<RawMag, Error> {
        ...
        if self.verbose {
            ...
            debug!(
                "BMM350 raw buf (first6) {=[u8]} valid {=[u8]}",
                first_six, data_slice
            );
        }
    ...
    pub async fn read_ut<D: DelayMs>(&mut self, delay: &mut D) -> Result<(f32, f32, f32), Error> {
        ...
        Ok((
            raw.xyz[0] as f32 * scale,
            raw.xyz[1] as f32 * scale,
            raw.xyz[2] as f32 * scale,
        ))
    }
    ...
    async fn load_otp<D: DelayMs>(&mut self, delay: &mut D) -> Result<(), Error> {
        ...
        if self.verbose {
            ...
            debug!(
                ...
                first_six, buf[22], self.otp.data[0]
            );
        }
        Ok(())
    }
    ...
    async fn wait_not_busy<D: DelayMs>(
        &mut self,
        delay: &mut D,
        timeout_ms: u32,
    ) -> Result<(), Error> {
...
fn atan_approx(z: f32) -> f32 {
    let abs_z = z.abs();
    if abs_z > 1.0 {
        return core::f32::consts::FRAC_PI_2 - atan_approx(1.0 / abs_z);
    }
    let zz = abs_z * abs_z;
    let mut res = abs_z;
