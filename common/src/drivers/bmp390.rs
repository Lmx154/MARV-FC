    ...
    I2C: I2c,
{
    ...
    #[inline]
    async fn wait_cmd_ready<D: DelayMs>(
        &mut self,
        delay: &mut D,
        mut tries: u8,
    ) -> Result<(), Error> {
    ...
    #[inline]
    async fn ensure_normal_mode<D: DelayMs>(
        &mut self,
        delay: &mut D,
        retries: u8,
    ) -> Result<(), Error> {
        ...
        while left > 0 {
            ...
            if (p & PWR_CTRL_NORMAL_MODE) == PWR_CTRL_NORMAL_MODE {
                return Ok(());
            }
            let _ = self.wait_cmd_ready(delay, 50).await;
            self.write_reg(REG_PWR_CTRL, PWR_CTRL_TEMP_PRESS_EN | PWR_CTRL_NORMAL_MODE)
                .await?;
    ...
    pub async fn init<D: DelayMs>(&mut self, delay: &mut D) -> Result<(), Error> {
        ...
        while retries > 0 {
            match self.read_reg(REG_CHIP_ID).await {
                Ok(v) => {
                    id = v;
                    if id == CHIP_ID_388 || id == CHIP_ID_390 {
                        break;
                    }
                }
                Err(_) => {}
        ...
        bmp_dbg!(
            self,
            "After reset: {} CHIP_ID=0x{:02X} PWR=0x{:02X} STATUS=0x{:02X}",
            model,
            id,
            pwr0,
            stat0
        ...
        bmp_dbg!(
            self,
            "calib t1={:?} t2={:?} t3={:?} p1={:?} p2={:?} p5={:?}",
            self.calib.par_t1 as f32,
            self.calib.par_t2 as f32,
            self.calib.par_t3 as f32,
            self.calib.par_p1 as f32,
            self.calib.par_p2 as f32,
            self.calib.par_p5 as f32
        );
    ...
    pub async fn read_compensated<D: DelayMs>(
        &mut self,
        delay: &mut D,
    ...
    async fn read_compensated_ready(&mut self) -> Result<(i32, i32), Error> {
        ...
        let t_i32 = if t_c_x100.is_finite() {
            (t_c_x100 + if t_c_x100 >= 0.0 { 0.5 } else { -0.5 }) as i32
        } else {
            0
        };
        let p_i32f = p_pa;
        let p_i32 = if p_i32f.is_finite() {
            (p_i32f + if p_i32f >= 0.0 { 0.5 } else { -0.5 }) as i32
        } else {
            0
        };
